...
from model import create_Model,create_Unet
...
#model = create_Unet()
model.compile(optimizer=Adam(lr=1e-3),loss=loss, metrics=['categorical_accuracy'])

modelSavePath = 'models/' + Notes + '_' + trainingRunTime +  '_batchsize_' + str(args.batch) + '_resnet_' + str(args.resnet.value) + '/_weights_epoch{epoch:02d}_val_loss_{val_loss:.4f}_valacc_{val_categorical_accuracy:.4f}_train_loss_{loss:.4f}.hdf5'

os.makedirs('Z:/PYTHON/work/RoadSeg-copy/acc_and_loss/' + Notes + '_' + trainingRunTime + '_batchsize_' + str(args.batch) + '_resnet_' + str(args.resnet.value) + '/')

imsavepath1 = 'Z:/PYTHON/work/RoadSeg-copy/acc_and_loss/'+ Notes + '_' + trainingRunTime +  '_batchsize_' + str(args.batch) + '_resnet_' + str(args.resnet.value)+ '/Training and validation loss.jpg'
imsavepath2 = 'Z:/PYTHON/work/RoadSeg-copy/acc_and_loss/'+ Notes + '_' + trainingRunTime +  '_batchsize_' + str(args.batch) + '_resnet_' + str(args.resnet.value)+ '/Training and validation acc.jpg'
...
history = model.fit(train_generator, epochs=args.epochs, validation_data=test_generator, callbacks=[mc,mc1,lr,tb], initial_epoch=0)
...
def draw(history):
     ...
     plt.figure()
     epochs = range(1, len(history.history['categorical_accuracy']) + 1,1)
     plt.plot(epochs, history.history['categorical_accuracy'], 'r', label='Training acc')
     plt.plot(epochs, history.history['val_categorical_accuracy'], 'b', label='validation acc')
